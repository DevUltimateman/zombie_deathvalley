...
give_perk( perk )
{
	gun = self maps\_zombiemode_perks_custom::perk_give_bottle_begin( perk );
	self waittill_any( "fake_death", "death", "player_downed", "weapon_change_complete" );
	self maps\_zombiemode_perks_custom::perk_give_bottle_end( gun, perk );
	self maps\_zombiemode_perks_custom::give_perk( perk, true );
}
...
wunderfizz_bottle_spawn( wunderfizz, player )
{
	...
	for( i = 0; i < number_cycles; i++ )
	{
		...
		modelname = GetWeaponModel( "zombie_perk_bottle", level.zombie_perks[ rand ].perk_bottle );
