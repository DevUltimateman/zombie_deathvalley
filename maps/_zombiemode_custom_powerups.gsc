...
init()
{
	...
	set_zombie_var( "zombie_powerup_points_use", 				true );
	set_zombie_var( "zombie_powerup_papgun_use", 				false );
	set_zombie_var( "zombie_powerup_freeperk_use", 				false );
	set_zombie_var( "zombie_powerup_invincible_use", 				false );
	set_zombie_var( "zombie_powerup_invincible_time", 				20 );
	set_zombie_var( "zombie_powerup_speed_use", 				false );
	set_zombie_var( "zombie_powerup_speed_time", 				30 );
	set_zombie_var( "zombie_powerup_thunder_use", 				false );
	set_zombie_var( "zombie_powerup_thunder_time", 				20 );
	set_zombie_var( "zombie_powerup_unlimited_use", 				false );
...
init_powerups()
{
	...
	if( level.zombie_vars[ "zombie_powerup_freeperk_use" ] )
		add_zombie_powerup( "freeperk", "zombie_icon_perk" );
...
free_perk_powerup()
{
	...
	for( i = 0; i < players.size; i++ )
	{
		if ( !players[i] maps\_laststand::player_is_in_laststand() && !(players[i].sessionstate == "spectator") )
		{
			...
			if( perk_name != "" )
			{
				players[i] thread maps\_zombiemode_perks_custom::give_perk( perk_name );
			}
		}
	}
}
