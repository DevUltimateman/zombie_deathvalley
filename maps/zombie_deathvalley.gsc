...
main()
{
	...
	maps\zombie_deathvalley_fx::main();
	maps\zombie_deathvalley_amb::main();
	...
	level.zombie_anim_override = maps\zombie_deathvalley::anim_override_func;
...
{
	switcher = getent( "trig_env1", "targetname" );
	switcher setHintString( "THIS IS A TEST AMB EFFECT" );
	switcher useTriggerRequireLookAt();
	switcher setCursorHint("HINT_NOICON");
	list = [];
	list[ list.size ] = "creek_1_rain";
	...
	list[ list.size ] = "river_land";
	list[ list.size ] = "river_plane";
	list[ list.size ] = "underwaterbase_swimming";
	list[ list.size ] = "vampire_high";
	list[ list.size ] = "zombie_deathvalley";
	x = 0;
	while( true )
	{
		if( x > list.size )
		...
		iPrintLnBold( "SOMEONE ACCESSED TRIGGER LOOP TO ANOTHER VISION FILE" );
		//ghostship_fog();
		visionSetNaked( list[ x ], 0.5 );
		x++;
		wait 4;
	}
}
...
zombie_deathvalley_trigger_multiple_vision_test()
{
	...
	switcher setCursorHint("HINT_NOICON");
	fog_list = [];
	fog_list[ fog_list.size ] = "lab_exterior_fog";
	fog_list[ fog_list.size ] = "start_crate_lowered_fog";
	fog_list[ fog_list.size ] = "btr_rail_fog";
	fog_list[ fog_list.size ] = "rooftop_fog";
	fog_list[ fog_list.size ] = "gas_attack_enter_fog";
	s = 0;
	list = [];
	list[ list.size ] = "creek_1_rain";
	list[ list.size ] = "cuba_streets_floodlights";
	...
	while( true )
	{
		...
		switcher waittill( "trigger", who );
		iPrintLnBold( "SOMEONE ACCESSED TRIGGER LOOP TO ANOTHER VISION FILE" );
		iprintln( "Vision set to: ^8" + list[ x ] );
		//ghostship_fog();
		visionSetNaked( list[ x ], 0.5 );
		x++;
		wait 4;
	...
	while( true )
	{
		...
		iPrintLnBold( "^2VISION CHANGED TO: ^8" + list[ a ] );
		visionSetNaked( list[ a ], 0 );
		a++;
		wait 5;
	}
	*/
...
new_test_fog()
{
	start_dist = 257;
	half_dist = 637;
	half_height = 1500; //water level height of zm_deathvalley 1800
	base_height = -250; //initial test zone height of zm_deathvalley
	fog_r = 0.12;
	fog_g = 0.65;
	fog_b = 0.45;
	fog_scale = 1;
	...
	VisionSetNaked( "fullahead_betrayal", 0 );
...
ghostship_fog()
{
	...
	visionSetNaked( "fullahead_nova6", 0 );
...
zombie_deathvalley_ignore_spawner( spawner )
{
	// no power, no quads
	if ( !flag("power_on") )
	{
...
zombie_deathvalley_exit_level()
{
	zombies = GetAiArray( "axis" );
	for ( i = 0; i < zombies.size; i++ )
	{
		zombies[i] thread zombie_deathvalley_find_exit_point();
	}
}

zombie_deathvalley_find_exit_point()
{
	self endon( "death" );
...
weather_control()
{
	...
	rainInit( "light" ); // get rain going ( rains: "hard", "medium", "light" )
